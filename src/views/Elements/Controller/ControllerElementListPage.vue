          ...
          <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
              <DataTable
                  ...
                  :globalFilterFields="['code','name','oem.description','technology.description','branch.country.name','branch.company.description','status']"
              ...
              >
                <template #header>
                  <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                      <h1 class="text-xl font-semibold text-gray-900">Controller elements</h1>
                      <p class="mt-2 text-sm font-normal text-gray-700">A list of all the controller elements available.</p>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 inline-flex items-center sm:flex-none">
                      <InputText v-model="filters['global'].value" placeholder="Keyword Search" />
                      <router-link to="/elements/controllers/create" class="ml-4 inline-flex items-center justify-center rounded-md border border-transparent bg-amerinode-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-amerinode-blue-700 focus:outline-none focus:ring-2 focus:ring-amerinode-blue-500 focus:ring-offset-2 sm:w-auto">
                ...
                <Column field="oem.description" header="OEM" sortable style="min-width: 140px; max-width: 140px;">
                  <template #body="{data}">
                ...
                <Column field="technology.description" header="Technology" sortable style="min-width: 195px; max-width: 195px;">
                  <template #body="{data}">
                ...
                <Column :exportable="false">
                  <template #body="slotProps">
                    <div class="ml-4 flex items-center md:ml-6">
                      <router-link :to="{ name: 'ControllerElementShow', params: { controllerElementId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
                        <EyeIcon class="h-6 w-6" aria-hidden="true" />
                        <span class="sr-only">show</span>
                      </router-link>
                      <router-link :to="{ name: 'ControllerElementEdit', params: { controllerElementId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
              ...
              <dialog-confirm
                  ...
                  :title="'Inactive controller element'"
...
import controllerElementsService from '@/services/controller_element.service.js'
import { notifyError } from '@/utils/notify_error'
import { notifySuccess } from '@/utils/notify_success'

const controllerElements = ref()
const controllerElement = ref({})
...
const trace = [
  ...
  { description: 'Controller', pathName: 'ControllerElementList', isLink: true, current: true }
]
...
const filters = ref({
  ...
  'technology.description': {operator: FilterOperator.AND, constraints: [{value: null, matchMode: FilterMatchMode.CONTAINS}]},
...
function loadData() {
  loading.value = true;
  controllerElementsService.list().then(x => {
    controllerElements.value = x.data
    loading.value = false
  })
...
function confirmDelete(data) {
  controllerElement.value = data
  messageDelete.value = "Are you sure you want to inactive the controller element '"+controllerElement.value.name+"'?"
  openDialogConfirm.value = true
}

const deleteRegister = () => {
  closeDeleteDialog()
  controllerElementsService.delete(controllerElement.value.id).then(x => {
