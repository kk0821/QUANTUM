      ...
      <div class="md:grid md:grid-cols-4 md:gap-6">
        <div class="md:col-span-1">
          <div class="px-4 sm:px-0">
            ...
            <p class="mt-1 text-sm text-gray-600">Descriptive information of the site</p>
          </div>
        </div>
              ...
              <div class="px-4 py-5 bg-white sm:p-6">
                <div class="grid grid-cols-6 gap-6">
                  ...
                  <div class="col-span-6 lg:col-span-3">
                    <base-input
                        :id="'ip-site'"
                        :autocomplete="'ip-site'"
                        v-model="site.ip_address"
                        label="IP Address"
                        type="text"
                        :is-disabled="!canEdit"
                        :is-required="true"
                  ...
                  <div class="col-span-6 lg:col-span-3">
                    <base-input
                        :id="'technology-site'"
                        :autocomplete="'technology-site'"
                        v-model="site.technology"
                        label="Technology"
                        type="text"
                        :is-disabled="!canEdit"
                        :is-required="true"
      ...
      <div class="md:grid md:grid-cols-4 md:gap-6">
        <div class="md:col-span-1">
          <div class="px-4 sm:px-0">
            ...
            <p class="mt-1 text-sm text-gray-600">Configuration of the branch where the site is located.</p>
          </div>
        </div>
        <div class="mt-5 md:mt-0 md:col-span-3">
          <form action="#" method="POST">
...
const loadCompany = (setOld) => {
  companyService.byCountry(selectedCountry.value.id).then(x => {
    ...
    if (setOld) {
      useSetInitialSelect(companies.value, data.site.value.branch.company.id, selectedCompany)
    } else {
...
onBeforeMount(() => {
  ...
  resolver.all({
    ...
    if (data.site.value.id) {
      useSetInitialSelect(statuses.value, data.site.value.status, selectedStatus)
      useSetInitialSelect(listOem.value, data.site.value.oem?.id, selectedOem)
      useSetInitialSelect(countries.value, data.site.value.branch?.country.id, selectedCountry)
      loadCompany(true)
    } else {
