          ...
          <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
              <DataTable
                  ...
                  :globalFilterFields="['name','technology.description','oem.description','status']"
              ...
              >
                <template #header>
                  <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                      ...
                      <p class="mt-2 text-sm font-normal text-gray-700">A list of all the network elements available.</p>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 inline-flex items-center sm:flex-none">
                      <InputText v-model="filters['global'].value" placeholder="Keyword Search" />
                      <router-link to="/elements/networks/create" class="ml-4 inline-flex items-center justify-center rounded-md border border-transparent bg-amerinode-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-amerinode-blue-700 focus:outline-none focus:ring-2 focus:ring-amerinode-blue-500 focus:ring-offset-2 sm:w-auto">
                ...
                <Column field="technology.description" header="Technology" sortable class="min-w-[10rem]">
                  <template #body="{data}">
                ...
                <Column field="oem.description" header="OEM" sortable class="min-w-[10rem]">
                  <template #body="{data}">
                ...
                <Column :exportable="false">
                  <template #body="slotProps">
                    <div class="ml-4 flex items-center md:ml-6">
                      <router-link :to="{ name: 'NetworkElementShow', params: { networkElementId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
                        <EyeIcon class="h-6 w-6" aria-hidden="true" />
                        <span class="sr-only">show</span>
                      </router-link>
                      <router-link :to="{ name: 'NetworkElementEdit', params: { networkElementId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
              ...
              <dialog-confirm
                  ...
                  :title="'Inactive network element'"
...
const trace = [
  ...
  { description: 'Network', pathName: 'NetworkElementList', isLink: true, current: true }
]
const statuses = ref([
  'active', 'inactive'
]);
...
const loadData = () => {
  networkElementsService.list().then(x => {
    networkElements.value = x.data
  })
...
const confirmDelete = (data) => {
  networkElement.value = data
  messageDelete.value = "Are you sure you want to inactive the network element '"+networkElement.value.name+"'?"
  openDialogConfirm.value = true
}

const deleteRegister = () => {
  closeDeleteDialog()
  networkElementsService.delete(networkElement.value.id).then(x => {
