      ...
      <div class="md:grid md:grid-cols-3 md:gap-6">
        <div class="md:col-span-1">
          <div class="px-4 sm:px-0">
            ...
            <p class="mt-1 text-sm text-gray-600">Descriptive information of the location</p>
          </div>
        </div>
...
const loadCompany = (setOld) => {
  companyService.byCountry(selectedCountry.value.id).then(x => {
    ...
    if (setOld) {
      useSetInitialSelect(companies.value, data.location.value.branch.company.id, selectedCompany)
    } else {
...
onBeforeMount(() => {
  ...
  countryService.list()
    .then(resp => {
      ...
      if (data.location.value.id) {
        useSetInitialSelect(statuses.value, data.location.value.status, selectedStatus)
        useSetInitialSelect(countries.value, data.location.value.branch?.country.id, selectedCountry)
        loadCompany(true)
      } else {
