        ...
        <div class="mt-5 md:mt-0 md:col-span-2">
          <form action="#" method="POST">
            <div class="shadow sm:rounded-md">
              <div class="px-4 py-5 bg-white sm:p-6">
                <div class="grid grid-cols-6 gap-6">
                  <div v-show="canEdit" class="col-span-6 sm:col-span-3 lg:col-span-3">
                    <base-input
                        :id="'password'"
                  ...
                  <div class="col-span-6 sm:col-span-3">
                    <Listbox as="div" v-model="selectedRole" :disabled="!canEdit">
...
onBeforeMount( () => {
  ...
  resolver.all({
    ...
    roles.value = resp.roles.data
    if (data.user.value.id) {
      useSetInitialSelect(statuses.value, data.user.value.status, selectedStatus)
      useSetInitialSelect(roles.value, data.user.value.roles[0].id, selectedRole)
    } else {
      useSetFirstElement(statuses.value, selectedStatus)
      useSetFirstElement(roles.value, selectedRole)
    }
    loading.hide()
  })
})
