        ...
        <div class="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
              <DataTable :value="permissions" :paginator="true" :rows="5"
                ...
                <template #header>
                  <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                      ...
                      <p class="mt-2 text-sm font-normal text-gray-700">A list of all the permissions.</p>
                      <p class="mt-2 text-xs font-normal text-gray-700 italic"><span class="font-semibold">Warning:</span> The changes you make may affect the operation of the application</p>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 inline-flex items-center sm:flex-none">
                      <InputText v-model="filters['global'].value" placeholder="Keyword Search" />
                      <router-link to="/permissions/create" class="ml-4 inline-flex items-center justify-center rounded-md border border-transparent bg-amerinode-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-amerinode-blue-700 focus:outline-none focus:ring-2 focus:ring-amerinode-blue-500 focus:ring-offset-2 sm:w-auto">
                ...
                <Column field="guard_name" header="Guard Name" sortable style="min-width: 14rem">
                  <template #body="{data}">
                    {{data.guard_name}}
                  </template>
                  <template #filter="{filterModel}">
                ...
                <Column :exportable="false">
                  <template #body="slotProps">
                    <div class="ml-4 flex items-center md:ml-6">
                      <router-link :to="{ name: 'PermissionShow', params: { permissionId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
                        <EyeIcon class="h-6 w-6" aria-hidden="true" />
                        <span class="sr-only">show</span>
                      </router-link>
                      <router-link :to="{ name: 'PermissionEdit', params: { permissionId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
                        <PencilIcon class="h-6 w-6" aria-hidden="true" />
                        <span class="sr-only">edit</span>
                      </router-link>
                      <button v-show="slotProps.data.status !== 'inactive'" type="button" @click="confirmDeletePermission(slotProps.data)" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
              ...
              <dialog-confirm
                  ...
                  :message="messageDeletePermission"
                  @executeAction="deletePermission"
                  @closeDialog="closeDeletePermissionDialog"
              />
            </div>
          </div>
        </div>
...
let messageDeletePermission = ref("")
let trace = [
  { description: 'Home', pathName: 'HomePage', isLink: true, current: false },
  { description: 'Permissions', pathName: 'PermissionList', isLink: true, current: true }
]
let statuses = ref([
  'active', 'inactive'
]);
...
function confirmDeletePermission(data) {
  permission.value = data
  messageDeletePermission.value = "Are you sure you want to inactive the permission "+permission.value.name+"?"
  openDialogConfirm.value = true
}

const deletePermission = () => {
  closeDeletePermissionDialog()
  permissionService.delete(permission.value.id).then(x => {
    notify({
      group: "top",
      title: "Delete",
      text: x.data,
