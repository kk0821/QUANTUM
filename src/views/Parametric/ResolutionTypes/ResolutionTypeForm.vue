      ...
      <div class="md:grid md:grid-cols-3 md:gap-6">
        <div class="md:col-span-1">
          <div class="px-4 sm:px-0">
            ...
            <p class="mt-1 text-sm text-gray-600">Descriptive information of the resolution type.</p>
          </div>
        </div>
              ...
              <div class="px-4 py-5 bg-white sm:p-6">
                <div class="grid grid-cols-6 gap-6">
                  ...
                  <div class="col-span-6 lg:col-span-3">
                    <Listbox as="div" v-model="selectedFailureType" :disabled="!canEdit">
                      <ListboxLabel class="block text-sm font-medium text-gray-700"> Failure type </ListboxLabel>
                      <div class="relative mt-1">
                        <ListboxButton class="bg-white relative w-full border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-amerinode-blue-500 focus:border-amerinode-blue-500 sm:text-sm"
                          ...
                          <span class="block truncate">{{ selectedFailureType.description }}</span>
                        ...
                        >
                          <ListboxOptions class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                            <ListboxOption
                                ...
                                v-for="failure_type in failureTypes"
                                :key="failure_type.id"
                                :value="failure_type"
                                as="template"
                            >
                              <li :class="[active ? 'bg-amerinode-blue-100 text-amerinode-blue-900' : 'text-gray-900','relative cursor-default select-none py-2 pl-10 pr-4']">
                                <span :class="[selected ? 'font-medium' : 'font-normal','block truncate']">{{ failure_type.description }}</span>
                  ...
                  <div class="col-span-6 lg:col-span-3">
                    <base-input
                        :id="'code-resolution-type'"
                        :autocomplete="'code-resolution-type'"
                        v-model="resolutionType.code"
                        label="Code"
                        type="text"
                        :is-disabled="!canEdit"
                        :is-required="true"
                  ...
                  <div class="col-span-6 lg:col-span-3">
                    <base-input
                        :id="'description-resolution-type'"
                        :autocomplete="'description-resolution-type'"
                        v-model="resolutionType.description"
                        label="Description"
                        type="text"
                        :is-disabled="!canEdit"
                        :is-required="true"
...
import failureTypeService from "@/services/failure_type.service";
...
const selectedFailureType = ref({})
const failureTypes = ref()

onBeforeMount(() => {
  loading.show()
  failureTypeService.list()
  .then(resp => {
    failureTypes.value = resp.data
    if (data.resolutionType.value.id) {
      useSetInitialSelect(statuses.value, data.resolutionType.value.status, selectedStatus)
      useSetInitialSelect(failureTypes.value, data.resolutionType.value.failure_type?.id, selectedFailureType)
    } else {
      useSetFirstElement(failureTypes.value, selectedFailureType)
...
const enableSave = computed(() => {
  return data.resolutionType.value.code
      && data.resolutionType.value.description
      && selectedFailureType.value.id
})

const sendData = (event) => {
  event.preventDefault()
  emit('save', {
    failure_type_id: selectedFailureType.value.id,
    resolution_type: data.resolutionType.value,
