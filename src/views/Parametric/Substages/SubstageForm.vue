      ...
      <div class="md:grid md:grid-cols-3 md:gap-6">
        <div class="md:col-span-1">
          <div class="px-4 sm:px-0">
            ...
            <p class="mt-1 text-sm text-gray-600">Descriptive information of the substage.</p>
          </div>
        </div>
              ...
              <div class="px-4 py-5 bg-white sm:p-6">
                <div class="grid grid-cols-6 gap-6">
                  ...
                  <div class="col-span-6">
                    <Listbox as="div" v-model="selectedEquipmentTypes" :disabled="!canEdit" multiple>
                      <ListboxLabel class="block text-sm font-medium text-gray-700"> Equipment type </ListboxLabel>
                      <div class="relative mt-1">
                        <ListboxButton class="bg-white relative w-full border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-amerinode-blue-500 focus:border-amerinode-blue-500 sm:text-sm"
                          ...
                          <span class="block truncate">
                            {{ selectedEquipmentTypes.map((x) => x.description).join(', ') }}
                          </span>
                        ...
                        >
                          <ListboxOptions class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                            <ListboxOption
                                ...
                                v-for="equipment_type in equipmentTypes"
                            ...
                            >
                              <li :class="[active ? 'bg-amerinode-blue-100 text-amerinode-blue-900' : 'text-gray-900','relative cursor-default select-none py-2 pl-10 pr-4']">
                                <span :class="[selected ? 'font-medium' : 'font-normal','block truncate']">{{ equipment_type.description }}</span>
                  ...
                  <div class="col-span-6 lg:col-span-3">
                    <base-input
                        :id="'code-substage'"
                        :autocomplete="'code-substage'"
                        v-model="substage.code"
                        label="Code"
                        type="text"
                        :is-disabled="!canEdit"
                        :is-required="true"
                  ...
                  <div class="col-span-6 lg:col-span-3">
                    <base-input
                        :id="'description-substage'"
                        :autocomplete="'description-substage'"
                        v-model="substage.description"
                        label="Description"
                        type="text"
                        :is-disabled="!canEdit"
                        :is-required="true"
...
import equipmentTypeService from "@/services/equipment_type.service";
...
onBeforeMount(() => {
  ...
  .then(resp => {
    equipmentTypes.value = resp.data
    if (data.substage.value.id) {
      const attachedIds = data.substage.value.equipment_types.map(x => x.id)
      selectedEquipmentTypes.value = equipmentTypes.value.filter(x => attachedIds.includes(x.id))
      useSetInitialSelect(statuses.value, data.substage.value.status, selectedStatus)
    } else {
      selectedEquipmentTypes.value.push(equipmentTypes.value[0])
...
const enableSave = computed(() => {
  return data.substage.value.code
      && data.substage.value.description
...
const sendData = (event) => {
  ...
  emit('save', {
    equipment_type_ids: selectedEquipmentTypes.value.map(x => x.id),
    substage: data.substage.value,
