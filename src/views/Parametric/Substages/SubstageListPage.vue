                ...
                <Column :exportable="false">
                  <template #body="slotProps">
                    <div class="ml-4 flex items-center md:ml-6">
                      <router-link :to="{ name: 'SubstageShow', params: { substageId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
                        <EyeIcon class="h-6 w-6" aria-hidden="true" />
                        <span class="sr-only">show</span>
                      </router-link>
                      <router-link :to="{ name: 'SubstageEdit', params: { substageId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
...
const trace = [
  ...
  { description: 'Substages', pathName: 'SubstagesList', isLink: false, current: true }
]
const statuses = ref([
  'active', 'inactive'
]);
...
function loadData() {
  substageService.list().then(x => {
    substages.value = x.data
  }).catch(err => {
...
function confirmDelete(data) {
  substage.value = data
  messageDelete.value = `Are you sure you want to inactive the substage ${substage.value.description}?`
  openDialogConfirm.value = true
}

const deleteRegister = () => {
  closeDeleteDialog()
  substageService.delete(substage.value.id).then(x => {
    notify({
      group: "top",
      title: "Delete",
      text: x.data,
