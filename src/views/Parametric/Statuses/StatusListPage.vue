        ...
        <div class="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
              <DataTable :value="statuses" :paginator="true" :rows="5"
                ...
                <Column field="module" header="Module" sortable class="min-w-[10rem]">
                  <template #body="{data}">
                    {{data.module}}
                  </template>
                  <template #filter="{filterModel}">
                    <InputText type="text" v-model="filterModel.value" class="p-column-filter" placeholder="Search by module"/>
                  </template>
                </Column>
                <Column field="description" header="Description" sortable class="min-w-[10rem]">
                  <template #body="{data}">
                    {{data.description}}
                  </template>
                  <template #filter="{filterModel}">
                ...
                <Column field="stop_time" header="Stop time" class="min-w-[5rem]">
                  <template #body="{data}">
                    <InputSwitch v-model="data.stop_time" disabled />
                  </template>
                </Column>
...
let trace = [
  ...
  { description: 'Status', pathName: 'StatusList', isLink: true, current: true }
]
let statusesList = ref([
  'active', 'inactive'
]);
let filters = ref({
  'global': {value: null, matchMode: FilterMatchMode.CONTAINS},
  'code': {operator: FilterOperator.AND, constraints: [{value: null, matchMode: FilterMatchMode.STARTS_WITH}]},
  'module': {operator: FilterOperator.AND, constraints: [{value: null, matchMode: FilterMatchMode.STARTS_WITH}]},
...
function loadStatuses() {
  statusService.list().then(x => {
    statuses.value = x.data
    statuses.value.forEach(status => status.stop_time = (status.stop_time === 1))
  }).catch(err => {
...
function confirmDelete(data) {
  status.value = data
  messageDelete.value = "Are you sure you want to inactive the status "+status.value.description+"?"
  openDialogConfirm.value = true
}

const deleteStatus = () => {
  closeDeleteDialog()
  statusService.delete(status.value.id).then(x => {
    notify({
      group: "top",
      title: "Delete",
      text: x.data,
