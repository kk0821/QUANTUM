        ...
        <div class="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
              <DataTable :value="serviceTypes" :paginator="true" :rows="5"
                         ...
                         :globalFilterFields="['id','description']" responsiveLayout="scroll">
                <template #header>
                  <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                      <h1 class="text-xl font-semibold text-gray-900">Services types</h1>
                      <p class="mt-2 text-sm font-normal text-gray-700">A list of all the service type available.</p>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 inline-flex items-center sm:flex-none">
                      <InputText v-model="filters['global'].value" placeholder="Keyword Search" />
                      <router-link to="/parametric/service-types/create" class="ml-4 inline-flex items-center justify-center rounded-md border border-transparent bg-amerinode-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-amerinode-blue-700 focus:outline-none focus:ring-2 focus:ring-amerinode-blue-500 focus:ring-offset-2 sm:w-auto">
                ...
                <Column field="id" header="Id" sortable class="min-w-[5rem]">
                  <template #body="{data}">
                    {{data.id}}
                  </template>
                ...
                <Column field="code" header="Code" sortable class="min-w-[10rem]">
                  <template #body="{data}">
                    {{data.code}}
                  </template>
                  <template #filter="{filterModel}">
                    <InputText type="text" v-model="filterModel.value" class="p-column-filter" placeholder="Search by cpde"/>
                  </template>
                </Column>
                <Column field="description" header="Description" sortable class="min-w-[25rem]">
                  <template #body="{data}">
                    {{data.description}}
                  </template>
                  <template #filter="{filterModel}">
                ...
                <Column :exportable="false">
                  <template #body="slotProps">
                    <div class="ml-4 flex items-center md:ml-6">
                      <router-link :to="{ name: 'ServiceTypeShow', params: { serviceTypeId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
                        <EyeIcon class="h-6 w-6" aria-hidden="true" />
                        <span class="sr-only">show</span>
                      </router-link>
                      <router-link :to="{ name: 'ServiceTypeEdit', params: { serviceTypeId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
...
import serviceTypeService from "@/services/service_type.service.js"
...
let trace = [
  ...
  { description: 'Service types', pathName: 'ServiceTypeList', isLink: true, current: true }
]
let statuses = ref([
  'active', 'inactive'
]);
...
function loadData() {
  serviceTypeService.list().then(x => {
    serviceTypes.value = x.data
  }).catch(err => {
...
function confirmDelete(data) {
  ...
  messageDelete.value = "Are you sure you want to inactive the service type "+serviceType.value.description+"?"
  openDialogConfirm.value = true
}

const deleteItem = () => {
  closeDeleteDialog()
  serviceTypeService.delete(serviceType.value.id).then(x => {
    notify({
      group: "top",
      title: "Delete",
      text: x.data,
