          ...
          <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
              <DataTable
                ...
                :globalFilterFields="['country.name','city','branch.country.name','branch.company.description','status']"
              ...
              >
                  ...
                  <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                      <h1 class="text-xl font-semibold text-gray-900">
                        <span class="capitalize">{{sourceType}}</span> sources
                      </h1>
                      <p class="mt-2 text-sm font-normal text-gray-700">A list of all the {{sourceType}} sources available.</p>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 inline-flex items-center sm:flex-none">
                      <InputText v-model="filters['global'].value" placeholder="Keyword Search" />
                      <router-link :to="`/parametric/${sourceType}/source/create`" class="ml-4 inline-flex items-center justify-center rounded-md border border-transparent bg-amerinode-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-amerinode-blue-700 focus:outline-none focus:ring-2 focus:ring-amerinode-blue-500 focus:ring-offset-2 sm:w-auto">
                        <PlusIcon class="-ml-1 mr-2 h-5 w-5" aria-hidden="true" />
                        Add a {{sourceType}} source
                      </router-link>
                    </div>
                  </div>
                </template>
                <template #empty>
                  No {{sourceType}} sources found.
                </template>
                <template #loading>
                ...
                <Column field="city" header="City" sortable class="min-w-[15rem]">
                  <template #body="{data}">
                    <span class="text-xs">{{data.city}}</span>
                  </template>
                ...
                <Column :exportable="false">
                  <template #body="slotProps">
                    <div class="ml-4 flex items-center md:ml-6">
                      <router-link :to="{ name: 'CitySourceShow', params: { citySourceId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
                        <EyeIcon class="h-6 w-6" aria-hidden="true" />
                        <span class="sr-only">show</span>
                      </router-link>
                      <router-link :to="{ name: 'CitySourceEdit', params: { citySourceId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
...
const trace = [
  ...
  { description: `${sourceTypeCaption.value}`, pathName: 'CitySourceList', isLink: false, current: true }
]
const statuses = ref([
  'active', 'inactive'
]);
...
function loadData() {
  citySourceService.list(`${sourceType.value}_source`).then(x => {
    citySources.value = x.data
  }).catch(err => {
...
function confirmDelete(data) {
  citySource.value = data
  messageDelete.value = `Are you sure you want to delete the ${sourceType.value} source ${citySource.value.city}?`
  openDialogConfirm.value = true
}

const deleteItem = () => {
  closeDeleteDialog()
  citySourceService.delete(citySource.value.id).then(x => {
