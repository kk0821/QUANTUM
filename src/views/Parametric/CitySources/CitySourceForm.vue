      ...
      <div class="md:grid md:grid-cols-3 md:gap-6">
        <div class="md:col-span-1">
          <div class="px-4 sm:px-0">
            ...
            <p class="mt-1 text-sm text-gray-600">Descriptive information of the {{sourceType}} source.</p>
          </div>
        </div>
              ...
              <div class="px-4 py-5 bg-white sm:p-6">
                <div class="grid grid-cols-6 gap-6">
                  <div class="col-span-6 lg:col-span-3">
                    <base-input
                        :id="'city-source'"
                        :autocomplete="'city-source'"
                        v-model="citySource.city"
                        label="City"
                        type="text"
                        :is-disabled="!canEdit"
                        :is-required="true"
...
const sendData = (event) => {
  ...
  emit('save', {
    city: data.citySource.value.city,
...
const loadCompany = (setOld) => {
  companyService.byCountry(selectedCountry.value.id).then(x => {
    ...
    if (setOld) {
      useSetInitialSelect(companies.value, data.citySource.value.branch.company.id, selectedCompany)
    } else {
...
onBeforeMount(() => {
  ...
  countryService.list()
    .then(resp => {
      ...
      if (data.citySource.value.id) {
        useSetInitialSelect(statuses.value, data.citySource.value.status, selectedStatus)
        useSetInitialSelect(countries.value, data.citySource.value.branch?.country.id, selectedCountry)
        loadCompany(true)
      } else {
