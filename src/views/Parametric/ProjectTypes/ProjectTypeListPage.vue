        ...
        <div class="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
              <DataTable :value="projectTypes" :paginator="true" :rows="5"
                         ...
                         :globalFilterFields="['id','description', 'service_type.description']" responsiveLayout="scroll">
                <template #header>
                  <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                      <h1 class="text-xl font-semibold text-gray-900">Project types</h1>
                      <p class="mt-2 text-sm font-normal text-gray-700">A list of all the project type available.</p>
                    </div>
                ...
                <Column field="description" header="Description" sortable class="min-w-[20rem]">
                  <template #body="{data}">
                    {{data.description}}
                  </template>
                  <template #filter="{filterModel}">
                ...
                <Column field="service_type.description" header="Service type" sortable class="min-w-[20rem]">
                  <template #body="{data}">
                    {{data.service_type.description}}
                  </template>
                  <template #filter="{filterModel}">
                ...
                <Column :exportable="false">
                  <template #body="slotProps">
                    <div class="ml-4 flex items-center md:ml-6">
                      <router-link :to="{ name: 'ProjectTypeShow', params: { projectTypeId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
                        <EyeIcon class="h-6 w-6" aria-hidden="true" />
                        <span class="sr-only">show</span>
                      </router-link>
                      <router-link :to="{ name: 'ProjectTypeEdit', params: { projectTypeId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
...
import projectTypeService from "@/services/project_type.service.js"
...
let trace = [
  ...
  { description: 'Project types', pathName: 'ProjectTypeList', isLink: true, current: true }
]
let statuses = ref([
  'active', 'inactive'
]);
...
function confirmDelete(data) {
  ...
  messageDelete.value = "Are you sure you want to inactive the project type "+projectType.value.description+"?"
  openDialogConfirm.value = true
}

const deleteRegister = () => {
  closeDeleteDialog()
  projectTypeService.delete(projectType.value.id).then(x => {
    notify({
      group: "top",
      title: "Delete",
      text: x.data,
