        ...
        <div class="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
              <DataTable :value="severities" :paginator="true" :rows="5"
                         ...
                         :globalFilterFields="['code','name','description']" responsiveLayout="scroll">
                <template #header>
                  <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                      <h1 class="text-xl font-semibold text-gray-900">Severities</h1>
                      <p class="mt-2 text-sm font-normal text-gray-700">A list of all the severity available</p>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 inline-flex items-center sm:flex-none">
                      <InputText v-model="filters['global'].value" placeholder="Keyword Search" />
                      <router-link to="/parametric/severities/create" class="ml-4 inline-flex items-center justify-center rounded-md border border-transparent bg-amerinode-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-amerinode-blue-700 focus:outline-none focus:ring-2 focus:ring-amerinode-blue-500 focus:ring-offset-2 sm:w-auto">
                ...
                <Column field="code" header="Code" sortable class="min-w-[5rem] max-w-[10rem]">
                  <template #body="{data}">
                    <div href="#" class="flex-shrink-0 group block">
                      <div class="flex items-center">
                        <div class="mt-1">
                          <span class="inline-block h-8 w-8 rounded-full" v-bind:style="{ 'background-color': data.color}" />
                        </div>
                        <div class="ml-3">
                          <p class="text-sm font-medium">{{ data.code }}</p>
                        </div>
                      </div>
                    </div>
                  </template>
                ...
                <Column field="name" header="Name" sortable class="min-w-[5rem] max-w-[10rem]">
                  <template #body="{data}">
                    {{data.name}}
                  </template>
                  <template #filter="{filterModel}">
                ...
                <Column field="description" header="Description" sortable class="min-w-[10rem] max-w-[15rem]">
                  <template #body="{data}">
                    {{data.description}}
                  </template>
                  <template #filter="{filterModel}">
                ...
                <Column field="status" header="Status" sortable :filterMenuStyle="{'width':'14rem'}" class="min-w-[5rem] max-w-[10rem]">
                  <template #body="{data}">
                  ...
                  <template #filter="{filterModel}">
                    <Dropdown v-model="filterModel.value" :options="statusesList" placeholder="Any" class="p-column-filter" :showClear="true">
                ...
                <Column :exportable="false">
                  <template #body="slotProps">
                    <div class="ml-4 flex items-center md:ml-6">
                      <router-link :to="{ name: 'SeverityShow', params: { severityId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
                        <EyeIcon class="h-6 w-6" aria-hidden="true" />
                        <span class="sr-only">show</span>
                      </router-link>
                      <router-link :to="{ name: 'SeverityEdit', params: { severityId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
...
import severityService from "@/services/severity.service.js"
...
const trace = [
  ...
  { description: 'Severities', pathName: 'SeverityList', isLink: true, current: true }
]
...
function loadSeverities() {
  severityService.list().then(x => {
    severities.value = x.data
  })
...
function confirmDelete(data) {
  severity.value = data
  messageDelete.value = "Are you sure you want to inactive the severity "+severity.value.name+"?"
  openDialogConfirm.value = true
}

const deleteRegister = () => {
  closeDeleteDialog()
  severityService.delete(severity.value.id).then(x => {
    notifySuccess('Delete', x.data)
    loadSeverities()
  }).catch(err => { notifyError((err.code === 'ERR_BAD_REQUEST') ? err.response.data.message : err.message) })
}
