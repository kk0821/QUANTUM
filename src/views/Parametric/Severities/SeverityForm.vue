      ...
      <div class="md:grid md:grid-cols-3 md:gap-6">
        <div class="md:col-span-1">
          <div class="px-4 sm:px-0">
            ...
            <p class="mt-1 text-sm text-gray-600">Descriptive information of the severity</p>
          </div>
        </div>
...
const sendData = (event) => {
  ...
  emit('save', { data: data.severityData.value, color: selectedColor.value, status: selectedStatus.value } )
}
const changedColor = (event) => {
  selectedColor.value = event.selectedColor
}

onBeforeMount(() => {
  if (data.severityData.value.id) {
    useSetInitialSelect(statuses.value, data.severityData.value.status, selectedStatus)
  } else {
