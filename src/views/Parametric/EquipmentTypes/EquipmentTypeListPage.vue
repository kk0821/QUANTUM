                ...
                <template #header>
                  <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                      <h1 class="text-xl font-semibold text-gray-900">Equipment types</h1>
                      <p class="mt-2 text-sm font-normal text-gray-700">A list of all the equipment types available.</p>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 inline-flex items-center sm:flex-none">
                      <InputText v-model="filters['global'].value" placeholder="Keyword Search" />
                      <router-link to="/parametric/equipment-type/create" class="ml-4 inline-flex items-center justify-center rounded-md border border-transparent bg-amerinode-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-amerinode-blue-700 focus:outline-none focus:ring-2 focus:ring-amerinode-blue-500 focus:ring-offset-2 sm:w-auto">
                ...
                <Column :exportable="false">
                  <template #body="slotProps">
                    <div class="ml-4 flex items-center md:ml-6">
                      <router-link :to="{ name: 'EquipmentTypeShow', params: { equipmentTypeId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
                        <EyeIcon class="h-6 w-6" aria-hidden="true" />
                        <span class="sr-only">show</span>
                      </router-link>
                      <router-link :to="{ name: 'EquipmentTypeEdit', params: { equipmentTypeId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
...
const trace = [
  ...
  { description: 'Equipment types', pathName: 'EquipmentTypeList', isLink: false, current: true }
]
const statuses = ref([
  'active', 'inactive'
]);
...
function loadData() {
  equipmentTypeService.list().then(x => {
    equipmentTypes.value = x.data
  }).catch(err => {
...
function confirmDelete(data) {
  ...
  messageDelete.value = `Are you sure you want to inactive the equipment type ${equipmentType.value.code}?`
  openDialogConfirm.value = true
}

const deleteRegister = () => {
  closeDeleteDialog()
  equipmentTypeService.delete(equipmentType.value.id).then(x => {
    notify({
      group: "top",
      title: "Delete",
      text: x.data,
