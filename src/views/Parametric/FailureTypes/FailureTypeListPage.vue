          ...
          <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
              <DataTable
                ...
                :globalFilterFields="['id','description', 'service_type.description']"
                responsiveLayout="scroll"
              >
                <template #header>
                  <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                      <h1 class="text-xl font-semibold text-gray-900">Failure types</h1>
                      <p class="mt-2 text-sm font-normal text-gray-700">A list of all the failure types available.</p>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 inline-flex items-center sm:flex-none">
                      <InputText v-model="filters['global'].value" placeholder="Keyword Search" />
                      <router-link to="/parametric/failure-type/create" class="ml-4 inline-flex items-center justify-center rounded-md border border-transparent bg-amerinode-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-amerinode-blue-700 focus:outline-none focus:ring-2 focus:ring-amerinode-blue-500 focus:ring-offset-2 sm:w-auto">
                ...
                <Column :exportable="false">
                  <template #body="slotProps">
                    <div class="ml-4 flex items-center md:ml-6">
                      <router-link :to="{ name: 'FailureTypeShow', params: { failureTypeId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
                        <EyeIcon class="h-6 w-6" aria-hidden="true" />
                        <span class="sr-only">show</span>
                      </router-link>
                      <router-link :to="{ name: 'FailureTypeEdit', params: { failureTypeId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
...
const trace = [
  ...
  { description: 'Failure types', pathName: 'FailureTypeList', isLink: false, current: true }
]
const statuses = ref([
  'active', 'inactive'
]);
...
function loadData() {
  failureTypeService.list().then(x => {
    failureTypes.value = x.data
  }).catch(err => {
...
function confirmDelete(data) {
  failureType.value = data
  messageDelete.value = `Are you sure you want to inactive the failure type ${failureType.value.description}?`
  openDialogConfirm.value = true
}

const deleteRegister = () => {
  closeDeleteDialog()
  failureTypeService.delete(failureType.value.id).then(x => {
    notify({
      group: "top",
      title: "Delete",
      text: x.data,
