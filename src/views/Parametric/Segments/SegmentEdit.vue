...
const trace = [
  ...
  { description: 'Edit', pathName: 'SegmentEdit', isLink: false, current: true }
]
onBeforeMount( () => {
  SegmentService.edit(route.params.segmentId).then(x => {
    segmentData.value = x.data
    segmentName.value = x.data.description
  }).catch(err => notifyError((err.code === 'ERR_BAD_REQUEST') ? err.response.data.message : err.message) )
})

const saveData = (event) => {
  SegmentService.save(route.params.segmentId, {
