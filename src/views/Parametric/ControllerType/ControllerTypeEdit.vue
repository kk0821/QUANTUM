...
const trace = [
  ...
  { description: 'Edit', pathName: 'ControllerTypeEdit', isLink: false, current: true }
]
onBeforeMount( () => {
  ControllerTypesService.edit(route.params.controllerTypeId).then(x => {
    controllerTypeData.value = x.data
    controllerTypeName.value = x.data.description
  }).catch(err => notifyError((err.code === 'ERR_BAD_REQUEST') ? err.response.data.message : err.message) )
})

const saveData = (event) => {
  ControllerTypesService.save(route.params.controllerTypeId, {
