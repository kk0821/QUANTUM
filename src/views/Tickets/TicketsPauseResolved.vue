<template>
  <main>
    <div v-show="!loading.state" class="px-4 py-4 sm:px-6 lg:px-8">
      ...
      <FormPanel
        title="Pause resolved"
        sub-title="This action will stop the ticket service process until the requirements for it are completed."
        :rows="[
          { label: 'ticket-pause-resolved', col: 6 }
        ]"
        separation
      >
        <template #ticket-pause-resolved>
          <base-input
            id="ticket-pause-resolved"
...
const trace = useTrace({ description: 'Pause resolved', pathName: 'TicketsPauseResolved'})
const { canAction } = useCan()
const { ticketData, project, oldSelectors, oldFiles } = useLoadTicket(ticketId.value, (data) => {
  if (!canAction('pauseResolved', data)) {
...
const saveData = (event) => {
  ...
  loadingButton.show()
  ticketsService.pauseResolved(ticketId.value, {
