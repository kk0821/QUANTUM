    ...
    <div v-show="!loading.state" class="px-4 py-4 sm:px-6 lg:px-8">
      ...
      >
        <template #ticket-pending-info>
          <base-input
            id="ticket-pending-info"
            v-model="pendingInformation"
            label="Please indicate the reason why you are stopping progress"
            type="text"
            is-required
          />
        </template>
...
const trace = useTrace({ description: 'Stop progress', pathName: 'TicketsPending'})
const { canAction } = useCan()
const { ticketData, project, oldSelectors, oldFiles } = useLoadTicket(ticketId.value, (data) => {
  if (!canAction('pending', data)) {
    notifyError('The ticket is not available to stop progress or you do not have the permissions to perform this action')
    return router.push({ name: "TicketsList" })
  }
})

const enableSave = computed(() => {
  return !!(pendingInformation.value)
})

const saveData = (event) => {
  event.preventDefault()
  loadingButton.show()
  ticketsService.stopProgress(ticketId.value, {
    pendingInformation: pendingInformation.value
  }).then(x => {
