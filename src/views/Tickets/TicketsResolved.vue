<template>
  <main>
    <div v-show="!loading.state" class="px-4 py-4 sm:px-6 lg:px-8">
      ...
      <FormPanel
        title="Indicate resolved ticket"
        sub-title="Once the service is restored, it indicates the ticket is resolved, attaching the preliminary solution report."
        :rows="[
          { label: 'ticket-resolved-info-media', col: 6 },
          { label: 'ticket-resolved-info', col: 6 }
        ]"
        separation
      >
        <template #ticket-resolved-info-media>
          <TicketsFiles
            v-if="ticketData.id"
            title="Preliminary solution report"
        ...
        <template #ticket-resolved-info>
          <base-input
            id="ticket-resolved-info"
            v-model="resolvedInformation"
            label="Indicates a solution report comment."
            type="text"
            is-required
          />
        </template>
...
const resolvedInformation = ref('')
const ticketId = ref(route.params.ticketId)
const listFiles = ref([])

const trace = useTrace({ description: 'Resolved Ticket', pathName: 'TicketsResolved'})
const { canAction } = useCan()
const { ticketData, project, oldSelectors, oldFiles } = useLoadTicket(ticketId.value, (data) => {
  if (!canAction('resolved', data)) {
    notifyError('The ticket is not available to be indicated resolved or you do not have the permissions to perform this action')
...
const enableSave = computed(() => {
  return !!(resolvedInformation.value)
})

const saveData = (event) => {
  event.preventDefault()
  loadingButton.show()
  ticketsService.resolvedTicket(ticketId.value, {
    resolvedInformation: resolvedInformation.value,
