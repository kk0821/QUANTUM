  ...
  <main>
    <div v-show="!loading.state" class="px-4 py-4 sm:px-6 lg:px-8">
      ...
      <FormPanel
        ...
        sub-title="This action will resume the progress of attention to the ticket, which was previously stopped for some reason."
        :rows="[
          { label: 'ticket-pending-info', col: 6 },
          { label: 'ticket-pending-info-media', col: 6 },
          { label: 'ticket-inProgress-info', col: 6 }
        ]"
        separation
      >
        <template #ticket-pending-info>
          <base-input
            id="ticket-accept-user-info"
            v-model="pendingInformation"
            label="Reason why the process was stopped"
            type="text"
            is-disabled
          />
        </template>
        <template #ticket-pending-info-media>
          <TicketsFiles
            v-if="ticketData.id"
            title="Media support"
        ...
        <template #ticket-inProgress-info>
          <base-input
            id="ticket-inProgress-info"
            v-model="inProgressInformation"
            label="Indicates comment to continue progress"
            type="text"
            is-required
          />
        </template>
...
const pendingInformation = ref('')
const inProgressInformation = ref('')
const ticketId = ref(route.params.ticketId)
const listFiles = ref([])

const trace = useTrace({ description: 'Continue progress', pathName: 'TicketsInProgress'})
const { canAction } = useCan()
const { ticketData, project, oldSelectors, oldFiles } = useLoadTicket(ticketId.value, (data) => {
  if (!canAction('inProgress', data)) {
    notifyError('The ticket is not available to continue progress or you do not have the permissions to perform this action')
    return router.push({ name: "TicketsList" })
  }
  loading.show()
  ticketsService.lastPending(ticketId.value)
    .then(x => {
      if (x.data.pending) {
        pendingInformation.value = x.data.pending.note
      }
    ...
    .catch((err) => {
      loading.hide()
      notifyError((err.code === "ERR_BAD_REQUEST") ? err.response.data.message : 'An error occurred when consulting the information the last pending action')
    })
})
...
const enableSave = computed(() => {
  return !!(inProgressInformation.value)
})

const saveData = (event) => {
  event.preventDefault()
  loadingButton.show()
  ticketsService.continueProgress(ticketId.value, {
    inProgressInformation: inProgressInformation.value,
