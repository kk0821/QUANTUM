        ...
        <div class="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
              <DataTable :value="roles" :paginator="true" :rows="5"
                         dataKey="id" :rowHover="true" v-model:selection="selectedRoles" v-model:filters="filters" filterDisplay="menu" :loading="loading"
                ...
                <template #header>
                  <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                      ...
                      <p class="mt-2 text-sm font-normal text-gray-700">A list of all the roles.</p>
                    ...
                    <div class="mt-4 sm:mt-0 sm:ml-16 inline-flex items-center sm:flex-none">
                      ...
                      <router-link to="/roles/create" class="ml-4 inline-flex items-center justify-center rounded-md border border-transparent bg-amerinode-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-amerinode-blue-700 focus:outline-none focus:ring-2 focus:ring-amerinode-blue-500 focus:ring-offset-2 sm:w-auto">
                ...
                <Column :exportable="false">
                  <template #body="slotProps">
                    <div class="ml-4 flex items-center md:ml-6">
                      <router-link :to="{ name: 'RoleShow', params: { roleId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
                        <EyeIcon class="h-6 w-6" aria-hidden="true" />
                        <span class="sr-only">show</span>
                      </router-link>
                      <router-link :to="{ name: 'RoleEdit', params: { roleId: slotProps.data.id }}" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
                        <PencilIcon class="h-6 w-6" aria-hidden="true" />
                        <span class="sr-only">edit</span>
                      </router-link>
                      <button v-show="slotProps.data.status !== 'inactive'" type="button" @click="confirmDeleteRole(slotProps.data)" class="bg-white rounded-full p-1 mr-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amerinode-blue-500">
              ...
              <dialog-confirm
                  ...
                  :title="'Inactive role'"
                  :message="messageDeleteRole"
                  @executeAction="deleteRole"
...
const messageDeleteRole = ref("")
const trace = [
  { description: 'Home', pathName: 'HomePage', isLink: true, current: false },
  { description: 'Roles', pathName: 'RoleList', isLink: true, current: true }
]
const statuses = ref([
  'active', 'inactive'
]);
...
function confirmDeleteRole(data) {
  role.value = data
  messageDeleteRole.value = "Are you sure you want to inactive the role "+role.value.name+"?"
  openDialogConfirm.value = true
}

const deleteRole = () => {
  closeDeleteRoleDialog()
  roleService.delete(role.value.id).then(x => {
